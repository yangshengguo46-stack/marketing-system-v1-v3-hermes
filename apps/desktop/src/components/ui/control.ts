...
// compact radius, 12px text, padding-driven sizing (no fixed heights). The visual
...
export const controlVariants = cva(
  'desktop-input-chrome w-full min-w-0 rounded-[7px] border text-xs leading-4 text-foreground outline-none transition-[background-color,border-color,box-shadow] duration-[var(--mos-motion-fast)] ease-[var(--mos-ease-out)] placeholder:text-muted-foreground disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50',
  {
    variants: {
      size: {
