...
describe('Marketing OS business surfaces', () => {
  ...
  it('renders a real ratio-aware video workbench and records the final review', async () => {
    ...
    expect(screen.getByText('人物库')).toBeTruthy()
    expect(screen.getByText('当前项目还没有可用的人物素材')).toBeTruthy()
    expect(screen.getByRole('navigation', { name: '项目参考素材' })).toBeTruthy()
    fireEvent.click(screen.getByRole('button', { name: '关闭人物库' }))
    expect(screen.queryByText('人物库')).toBeNull()
    ...
    expect(screen.getByText('场景库')).toBeTruthy()
    expect(screen.getAllByText('品牌主视觉').length).toBe(2)
    expect(screen.getByText('Remotion')).toBeTruthy()
    const productionRail = inspectorRail.closest('aside')
    expect(productionRail).toBeTruthy()
    expect(within(productionRail as HTMLElement).getByText('作品状态')).toBeTruthy()
    expect(within(productionRail as HTMLElement).getByText('自动质检通过')).toBeTruthy()
    expect(within(productionRail as HTMLElement).getByText('无需检查')).toBeTruthy()
    const sceneLibraryPanel = screen.getByText('场景库').closest('aside')
    expect(sceneLibraryPanel).toBeTruthy()
    expect(within(sceneLibraryPanel as HTMLElement).queryByText('作品状态')).toBeNull()
    expect(within(sceneLibraryPanel as HTMLElement).queryByText('自动媒体质检')).toBeNull()
    expect(within(sceneLibraryPanel as HTMLElement).queryByRole('button', { name: '确认成片' })).toBeNull()
    expect(screen.queryByText('火山声音库')).toBeNull()
    fireEvent.click(within(inspectorRail).getByRole('button', { name: '声音' }))
    expect(await screen.findByText('声音库')).toBeTruthy()
    expect(screen.getByText('火山声音库')).toBeTruthy()
    expect(screen.getByText('常驻已接通')).toBeTruthy()
    expect(screen.getAllByText('火山声音库')).toHaveLength(1)
    expect(screen.queryByRole('dialog', { name: '选择旁白音色' })).toBeNull()
    ...
    expect(await screen.findByRole('button', { name: /小何 2.0/ })).toBeTruthy()
