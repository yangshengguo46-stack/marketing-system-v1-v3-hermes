...
The decision object is intentionally plain JSON so the Hermes agent, FastAPI,
Electron bridge and future UI cards can consume the same contract.
"""
...
from .influence_score import build_asset_influence_score, build_influence_score
...
def build_asset_preflight_decision(
    store: Any,
    asset_id: str,
    *,
    stage: str = "publish_review",
    metric_labels: dict[str, Any] | None = None,
    context: dict[str, Any] | None = None,
) -> dict[str, Any]:
    """Read asset features and return score + unified decision."""

    score = build_asset_influence_score(store, asset_id, metric_labels=metric_labels)
    decision = build_preflight_decision(score, stage=stage, context=context or {})
    return {
        "asset_id": asset_id,
