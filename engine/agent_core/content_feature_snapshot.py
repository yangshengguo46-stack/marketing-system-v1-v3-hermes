...
def _evidence_summary(evidence: Any) -> dict[str, Any]:
    ...
    with_url = [item for item in items if item.get("has_url") or item.get("url") or item.get("source_url")]
    titles: list[str] = []
    sources: list[str] = []
    for item in items[:8]:
        title = _text(item.get("title") or item.get("name") or item.get("headline"), limit=120)
        source = _text(item.get("url") or item.get("source_url") or item.get("link"), limit=200)
        if title:
            titles.append(title)
        if source:
...
def build_content_feature_snapshot(
    *,
    ...
    body: dict[str, Any] = {
        ...
        "retro_contract": {
            "label_dimensions": ["attention", "retention", "trust", "action", "fit", "risk"],
            "mutable": False,
