...
READ_TOOLS: list[ToolSpec] = [
    ...
    ToolSpec("marketing_read_account_onboarding", CapabilityLevel.READ_ONLY,
             "生成 Account DNA Onboarding 首次使用计划：用户建模、30 分钟可见成果、前三天激活节奏和下一步工具链；只读不写草案",
             _schema({
                 "account_id": {"type": "string", "description": "可选；无账号时使用 prospect 待绑定项目"},
                 "objective": {"type": "string", "description": "用户本轮目标或任务原文"},
                 "message": {"type": "string", "description": "用户自然语言输入"},
                 "business_goal": {"type": "string", "description": "业务、表达或变现目标"},
                 "industry": {"type": "string", "description": "行业/主题/赛道"},
                 "current_stage": {"type": "string", "description": "用户自述阶段"},
                 "resources": {"type": "array", "items": {"type": "string"}, "description": "经历、技能、案例、资源、观点等可供给资产"},
                 "audience": {"type": "array", "items": {"type": "string"}, "description": "用户自述想服务/影响的人群"},
                 "constraints": {"type": "array", "items": {"type": "string"}, "description": "不露脸、时间、隐私、争议边界等"},
                 "platforms": {"type": "array", "items": {"type": "string"}, "description": "目标平台，如 douyin/zhihu/wechat_mp"},
             }),
             _server_read("account_onboarding_plan", pass_params=True)),
