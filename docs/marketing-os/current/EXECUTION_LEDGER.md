...
- **声音库信息架构归位（2026-07-18）。** 生产阶段删除重复的“项目参考素材”五卡总览和第二层音色弹窗；右侧唯一 `VideoReferenceRail` 点击“声音”后直接打开声音库，Provider、音色与模型在同一临时面板内完成选择，点击“场景/人物/道具”同样直达各自分类库。项目旁白资产仍作为实际音轨回执保留，但不再用成片文件名冒充所选音色；作品状态、自动媒体质检、渲染门禁和“确认成片”继续保留在当前分类库下方。
...
- **导演台右侧工具轨归位（2026-07-18）。** 生产/成片阶段不再常驻 `20rem` 的“项目参考素材”详情栏，恢复设定阶段已经确认的 `7rem` 右侧工具轨，固定显示人物、声音、场景、道具四个入口。原本内嵌在 `VideoSetupWorkspace` 的窄轨已抽成唯一 `VideoReferenceRail`，设定与生产阶段共用同一份 `VIDEO_REFERENCE_CATEGORIES`、同一套缩略图与选中态；重复的 `SETUP_CATEGORIES`、`DirectorInspectorRail` 和后续遗留的五卡 `DirectorInspector` 总览路由层已删除。工具轨显示状态与分类库面板打开状态拆成两个独立 owner：点击入口只在工具轨左侧打开对应 `人物库/声音库/场景库/道具库` 的 `20rem` 临时浮层，不再修改 Grid 列宽，也不需要在浮层里重复选择分类；关闭浮层后工具轨仍保持 `7rem`，标题栏按钮只负责显示/隐藏工具轨。旧版混用 `inspectorRailCollapsed` 的 view-format v2 状态通过 v3 一次性迁移，避免旧常驻展开值污染新语义。目标 React 回归 `15/15`、TypeScript、目标 ESLint 和 `git diff --check` 通过；Electron 实机点击前后 Grid 均为 `240px 1203px 112px`，声音库无二次 Dialog，场景库仍保留作品状态、质检与确认成片门禁。
