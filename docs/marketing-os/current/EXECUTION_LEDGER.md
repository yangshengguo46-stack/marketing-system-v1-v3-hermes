...
- **导演台右侧工具轨归位（2026-07-18）。** 生产/成片阶段不再默认常驻 `20rem` 的“项目参考素材”详情栏，恢复设定阶段已经确认的 `7rem` 右侧工具轨，固定显示人物、声音、场景、道具四个入口；点击入口或标题栏右侧按钮时才展开原有详情、作品状态、自动质检和确认动作，再次折叠回窄轨。旧版持久化的常驻展开状态通过 view-format v2 一次性迁移为窄轨默认，之后继续记住用户手动选择。目标 React 结构与画幅回归 `15/15`、TypeScript 与目标 ESLint 通过。
