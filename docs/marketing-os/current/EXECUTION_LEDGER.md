...
- **导演台右侧工具轨归位（2026-07-18）。** 生产/成片阶段不再常驻 `20rem` 的“项目参考素材”详情栏，恢复设定阶段已经确认的 `7rem` 右侧工具轨，固定显示人物、声音、场景、道具四个入口。工具轨显示状态与详情面板打开状态已拆成两个独立 owner：点击入口只在工具轨左侧打开 `20rem` 临时浮层，不再修改 Grid 列宽；关闭浮层后工具轨仍保持 `7rem`，标题栏按钮只负责显示/隐藏工具轨。旧版混用 `inspectorRailCollapsed` 的 view-format v2 状态通过 v3 一次性迁移，避免旧常驻展开值污染新语义。目标 React 结构与画幅回归、TypeScript 与目标 ESLint 通过。
