...
- **成品预览画幅约束（2026-07-18）。** 本轮只调整中央成品 `<video>`，不改全局导航、镜头栏或右侧参考素材栏。最终媒体固定以 `object-contain + object-center` 完整显示，允许按画幅保留黑边，禁止为了铺满区域放大裁切；16:9 横屏预览上限由 `52rem` 收到 `44rem`，舞台最低高度由 `28rem` 收到 `24rem`，让时间线保留更多可视空间。成片态不再叠加单镜头时长角标，避免遮挡原生播放/全屏控制。桌面端画幅专项 `8/8` 与 TypeScript 全量检查通过。
