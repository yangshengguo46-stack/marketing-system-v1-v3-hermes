...
- **声音库信息架构归位（2026-07-18）。** 火山音色目录不再作为“项目参考素材”下方的第二套独立面板；该区域平时只保留一张紧凑“声音”素材卡和当前音色摘要，点击声音卡才弹出 Provider、音色与模型选择器，关闭后目录完全不占项目页空间。选择结果直接回填卡片摘要；项目旁白资产仍作为实际音轨回执保留，但不再用成片文件名冒充所选音色。
...
- **导演台右侧工具轨归位（2026-07-18）。** 生产/成片阶段不再常驻 `20rem` 的“项目参考素材”详情栏，恢复设定阶段已经确认的 `7rem` 右侧工具轨，固定显示人物、声音、场景、道具四个入口。原本内嵌在 `VideoSetupWorkspace` 的窄轨已抽成唯一 `VideoReferenceRail`，设定与生产阶段共用同一份 `VIDEO_REFERENCE_CATEGORIES`、同一套缩略图与选中态；重复的 `SETUP_CATEGORIES` 和 `DirectorInspectorRail` 已删除。工具轨显示状态与详情面板打开状态拆成两个独立 owner：点击入口只在工具轨左侧打开 `20rem` 临时浮层，不再修改 Grid 列宽；关闭浮层后工具轨仍保持 `7rem`，标题栏按钮只负责显示/隐藏工具轨。旧版混用 `inspectorRailCollapsed` 的 view-format v2 状态通过 v3 一次性迁移，避免旧常驻展开值污染新语义。目标 React 结构与画幅回归 `15/15`、TypeScript 与目标 ESLint 通过。
