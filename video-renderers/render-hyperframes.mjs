...
const visualMarkup = safeVisuals.map((visual, index) => {
  const inset = visual.presentation === 'inset_card'
    ? 'left:6%;right:6%;top:13%;bottom:43%;width:auto;height:auto;border-radius:36px;border:1px solid #ffffff3d;box-shadow:0 28px 80px #00000052;background:#08100eb8'
    : 'left:0;right:0;top:0;bottom:0;width:100%;height:100%';
  const style = `object-fit:${visual.fit};object-position:${visual.subjectAnchor};opacity:${index === 0 ? 1 : Math.max(0.2, 0.58 - index * 0.1)};${inset}`;
...
</style></head><body><div id="root" data-composition-id="main" data-start="0" data-duration="${Number(spec.duration)}" data-width="${Number(spec.canvas.width)}" data-height="${Number(spec.canvas.height)}" data-fps="${Number(spec.canvas.fps)}"><div class="visuals">${visualMarkup}</div><div class="shade"></div><div class="grid"></div><div class="top"><span class="tag">MARKETING OS</span></div><div class="copy">${headline ? `<div class="headline">${esc(headline.text)}</div>` : ''}${body ? `<div class="body">${esc(body.text)}</div>` : ''}<div class="progress"><div class="bar"></div></div></div><div class="wipe"></div></div><script>
