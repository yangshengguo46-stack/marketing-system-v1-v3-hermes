...
export type SceneProps = {
  ...
  visuals: Array<{
    ...
    sourceIn: number;
    fit: 'cover' | 'contain';
    presentation: 'full_bleed' | 'inset_card' | 'letterbox';
    subjectAnchor: 'center' | 'left' | 'right' | 'top' | 'bottom';
  }>;
...
const Visual = ({visual, index}: {visual: SceneProps['visuals'][number]; index: number}) => {
  ...
  const common: CSSProperties = {
    ...
    objectPosition: visual.subjectAnchor,
  ...
  return (
    <AbsoluteFill style={{backgroundColor: '#0b1110', color: '#fffaf0', fontFamily: font, opacity: exit}}>
      {props.visuals.map((visual, index) => (
        <AbsoluteFill
          key={`${visual.file}-${index}`}
          style={
            visual.presentation === 'inset_card'
              ? {
                  left: inset,
                  right: inset,
                  top: Math.round(height * 0.13),
                  bottom: Math.round(height * 0.43),
                  width: 'auto',
                  height: 'auto',
                  overflow: 'hidden',
                  borderRadius: Math.round(Math.min(width, height) * 0.035),
      ...
      <div style={{position: 'absolute', left: inset, right: inset, top: inset, display: 'flex', fontSize: Math.round(width * 0.022), letterSpacing: 4, fontWeight: 750}}>
