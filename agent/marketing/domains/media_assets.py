...
ASSET_ROLES = {"character", "voice", "scene", "prop", "storyboard", "broll", "music", "sfx", "other"}
SOURCE_TYPES = {"user_upload", "ai_generated", "volcengine_trusted", "licensed_provider"}
RIGHTS_STATUSES = {"user_confirmed", "provider_verified", "generated", "licensed"}
...
class MediaAssetRepository(MarketingDomainRepository):
        ...
        *,
        ...
        try:
            return self._insert(
                ...
                name=name, media_type=media_type, role=role, source_type=source_type,
                provider=provider, provider_asset_id=provider_asset_id,
                local_path=str(relative), mime_type=normalized_mime, sha256=digest,
                size_bytes=len(payload), rights_status=rights_status,
    ...
    def register_volcengine_trusted_asset(
        ...
        *,
        ...
        self._validate(media_type, "character", "volcengine_trusted", "provider_verified")
        metadata = _safe_json_object(metadata, "metadata")
        ...
        return self._insert(
            asset_id=f"media_{uuid.uuid4().hex}", user_id=user_id,
            account_id=account_id, name=name, media_type=media_type,
            role="character", source_type="volcengine_trusted",
            provider="volcengine_ark", provider_asset_id=provider_asset_id,
            local_path="", mime_type="", sha256="", size_bytes=0,
            rights_status="provider_verified", metadata=metadata, receipt={},
        )
        ...
        with self._transaction() as db:
            db.execute(
                ...
                (id,user_id,account_id,name,media_type,role,source_type,provider,
                 ...
                 metadata_json,receipt_json,status,created_at,updated_at)
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'active',?,?)""",
                (
                    values["asset_id"], values["user_id"], values["account_id"], values["name"],
                    values["media_type"], values["role"], values["source_type"], values["provider"],
                    values["provider_asset_id"], values["local_path"], values["mime_type"],
                    values["sha256"], values["size_bytes"], values["rights_status"],
                    json.dumps(values["metadata"], ensure_ascii=False),
                    json.dumps(values["receipt"], ensure_ascii=False), now, now,
                ),
            )
            row = db.execute("SELECT * FROM media_asset_library WHERE id=?", (values["asset_id"],)).fetchone()
        return _record(row)
        ...
        with self._connection() as db:
            if account_id:
                rows = db.execute(
                    ...
                    ORDER BY updated_at DESC""", (user_id, account_id),
                ).fetchall()
            else:
                rows = db.execute(
                    """SELECT * FROM media_asset_library
                    WHERE user_id=? AND status='active' ORDER BY updated_at DESC""", (user_id,),
                ).fetchall()
        return {"assets": [_record(row) for row in rows], "total": len(rows)}

    def add_reference(self, *, asset_id: str, user_id: str, owner_kind: str, owner_id: str, relation: str) -> None:
        ...
        with self._transaction() as db:
            db.execute(
                ...
                (f"ref_{uuid.uuid4().hex}", asset_id, owner_kind, owner_id, relation, _now()),
            )

    def remove_reference(self, *, asset_id: str, user_id: str, owner_kind: str, owner_id: str, relation: str) -> None:
        asset_id = _required_text(asset_id, "asset_id")
        user_id = _required_text(user_id, "user_id")
        owner_kind = _required_text(owner_kind, "owner_kind", limit=128)
        owner_id = _required_text(owner_id, "owner_id", limit=256)
    ...
    @staticmethod
    def _validate(media_type: str, role: str, source_type: str, rights_status: str) -> None:
